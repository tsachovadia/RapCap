...
if (API_KEY) {
    console.error(`🔑 Loaded API KEY. Length: ${API_KEY.length}, Prefix: ${API_KEY.substring(0, 4)}...`);
}
...
async function filterRhymesWithAI(words: string[]) {
    ...
    try {
        ...
        const filteredWords = JSON.parse(jsonString);
        console.log(`✨ AI selected ${filteredWords.length} words.`);
...
async function main() {
    ...
    // Save to results file in current directory
    const outputPath = path.join(process.cwd(), 'rhyme_results.json');
